...
//}
//
//pub trait AsEntityStruct<'a> {
//
//    fn as_entity_struct (&'a self) -> &'a EntityStruct;
//
//}
//
//pub trait EntityStruct<'a>: Send + Sync + EntityStructClone + EntityStructEq + EntityStructSerialize + AsEntityStruct<'a> {
//
//}
...
//impl <'a, S: Sized + Send + Sync + Eq + Clone + serde::Serialize + EntityStructClone + EntityStructEq + EntityStruct<'a> + AsEntityStruct<'a> + EntityStructSerialize>EntityStruct<'a> for S {
...
//        match sender {
//            Some (sender) => {
//                let item = self.item.unwrap().get_mut().as_entity_struct();
//                let item = self.item.take() as Option<Box<RefCell<EntityStruct>>>;
